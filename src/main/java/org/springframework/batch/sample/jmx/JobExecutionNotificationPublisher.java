 ...
 * @since 1.0
 */
public class JobExecutionNotificationPublisher implements ApplicationListener<SimpleMessageApplicationEvent>,
		NotificationPublisherAware
{
	...
	/**
	 * If the event is a {@link SimpleMessageApplicationEvent} for open and close we log the event at INFO level and
	 * send a JMX notification if we are also an MBean.
	 * 
	...
	private void publish(String message)
	{
		if (notificationPublisher != null)
		{
			Notification notification = new Notification("JobExecutionApplicationEvent", this, notificationCount++,
					message);
			/*
			 * We can't create a notification with a null source, but we can set it to null after creation(!). We want
			 * it to be null so that Spring will replace it automatically with the ObjectName (in
