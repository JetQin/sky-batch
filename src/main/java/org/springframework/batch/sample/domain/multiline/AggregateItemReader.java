...
/**
 * An {@link ItemReader} that delivers a list as its item, storing up objects from the injected {@link ItemReader} until
 * they are ready to be packed out as a collection. This class must be used as a wrapper for a custom {@link ItemReader}
 * that can identify the record boundaries. The custom reader should mark the beginning and end of records by returning
 * an {@link AggregateItem} which responds true to its query methods <code>is*()</code>.<br>
 * <br>
 * 
