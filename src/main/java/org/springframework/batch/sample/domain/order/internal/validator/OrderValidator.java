...
public class OrderValidator implements Validator
{
		...
		for (LineItem lineItem : lineItems)
		{
			...
			if (BD_MIN.compareTo(lineItem.getDiscountPerc()) != 0)
			{
				...
				if ((BD_MIN.compareTo(lineItem.getDiscountPerc()) > 0)
						...
						|| (BD_MIN.compareTo(lineItem.getDiscountAmount()) != 0))
				{ // only one of DiscountAmount and DiscountPerc should be non-zero
					discounts = false;
				}
			}
			else
			...
			BigDecimal coef = BD_100.subtract(lineItem.getDiscountPerc()).divide(BD_100, 4, BigDecimal.ROUND_HALF_UP);
			...
			BigDecimal singleItemPrice = discountedPrice.add(lineItem.getShippingPrice()).add(
					lineItem.getHandlingPrice());

			// total price = singleItemPrice * quantity
			BigDecimal quantity = new BigDecimal(lineItem.getQuantity());
			BigDecimal totalPrice = singleItemPrice.multiply(quantity).setScale(2, BigDecimal.ROUND_HALF_UP);
	...
	protected void validateAddress(Address address, Errors errors, String prefix)
	{
		if (address != null)
		{
			...
			if ((!StringUtils.hasText(address.getState()) && ("United States".equals(address.getCountry())) || StringUtils
					.hasText(address.getState()) && address.getState().length() != 2))
			{
