...
public class AggregateItemFieldSetMapper<T> implements FieldSetMapper<AggregateItem<T>>, InitializingBean
{
	...
	/**
	 * Public setter for the end field value. If the {@link FieldSet} input has a first field with this value that
	 * signals the start of an aggregate record.
	 * 
	...
	/**
	 * Public setter for the begin value. If the {@link FieldSet} input has a first field with this value that signals
	 * the end of an aggregate record.
	 * 
	...
	/**
	 * Build an {@link AggregateItem} based on matching the first column in the input {@link FieldSet} to check for
	 * begin and end delimiters. If the current record is neither a begin nor an end marker then it is mapped using the
	 * delegate.
